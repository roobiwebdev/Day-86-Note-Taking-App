...
const NoteSidebar: React.FC = () => {
  ...
  }, [isArchiveView, notes.length, archivedNotes.length, dispatch]);
  ...
  // Decide which notes to display based on the archive view
  ...
  return (
    <div className="flex flex-col items-start pl-7 pr-4 py-5 gap-4 border-r border-r-neutral-200 dark:border-r-slate-600 dark:bg-slate-950 w-[290px] h-screen">
      <button
        className="bg-blue-600 text-white font-medium text-center text-sm px-4 py-3 rounded-lg w-full hover:bg-blue-700"
      ...
      {displayedNotes.length === 0 ? (
        <>
          {isArchiveView && (
            <p className="text-neutral-500 dark:text-slate-400 text-[12px]">
          ...
          <div className="bg-neutral-100 dark:bg-slate-800 dark:border-slate-600 border border-neutral-200 p-2 rounded-lg flex flex-col items-center justify-center gap-2">
            ...
            <p className="text-neutral-500 dark:text-slate-400 text-[12px] font-normal">
              {isArchiveView
        ...
        <>
          {isArchiveView && (
            <p className="text-neutral-500 dark:text-slatel-400 text-[12px]">
              All your archived notes are stored here. You can restore or delete
              them anytime.
            </p>
          )}
          {selectedTag && (
            <p className="text-neutral-500 dark:text-slate-400 text-[12px]">
              All notes with the{" "}
              <span className="font-semibold"> "{selectedTag}"</span> tag are
              shown here.
            </p>
          )}
          <NoteContainer />
